...
import cn.wwah.common.myutils.Mail;
...
public class CrashHandlerUtil implements Thread.UncaughtExceptionHandler {
    ...
    private String saveCrashInfo2File(Throwable ex) {
        ...
        try {
            ...
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                ...
                if (filePath != null) {
                    JLog.d("filePath3333", filePath);
                    new Thread("A") {
                        @Override
                        public void run() {
                            boolean isSuccessMail = sendEmail(filePath);
                        }
                    }.start();
                }
            }
    ...
    // TODO 发送邮件的方法
    private boolean sendEmail(String filePath) {
        Mail m = new Mail("yujun666@foxmail.com", "zukkzwxcycfbbjjh");
        m.set_debuggable(false);
        String[] toArr = {"yujun999@foxmail.com"};
        m.set_to(toArr);
        m.set_from("yujun666@foxmail.com");
        m.set_subject("有BUG了，快点解决");
        m.setBody("有用户制造了BUG考验你，赶紧的");
        try {
